...
import (
	...
	"github.com/dominant-strategies/go-quai/internal/quaiapi/override"
	"github.com/dominant-strategies/go-quai/log"
	"github.com/dominant-strategies/go-quai/params"
	"github.com/dominant-strategies/go-quai/quai/tracers/logger"
...
type Backend interface {
	...
	StateAtBlock(ctx context.Context, block *types.WorkObject, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (*state.StateDB, StateReleaseFunc, error)
	StateAtTransaction(ctx context.Context, block *types.WorkObject, txIndex int, reexec uint64) (*types.Transaction, vm.BlockContext, *state.StateDB, StateReleaseFunc, error)
}
...
func (api *API) chainContext(ctx context.Context) core.ChainContext {
	return quaiapi.NewChainContext(ctx, api.backend)
}
...
func (api *API) traceChain(start, end *types.WorkObject, config *TraceConfig, closed <-chan error) chan *blockTraceResult {
...
func (api *API) traceBlockParallel(ctx context.Context, block *types.WorkObject, statedb *state.StateDB, config *TraceConfig) ([]*txTraceResult, error) {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				// concurrent use.
				// See: https://github.com/dominant-strategies/go-quai/issues/29114
				blockCtx := core.NewEVMBlockContext(block.Header(), api.chainContext(ctx), nil)
...
func (api *API) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	if err != nil {
		return nil, quaiapi.NewTxIndexingError()
	}
