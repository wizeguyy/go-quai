	...
	if nodeCtx == common.PRIME_CTX {
		...
		if !exists {
			...
			if block == nil {
				...
				c.CheckIfBlockIsAppended(block)
			}
		}
...
// CheckIfBlockIsAppended checks if block is appended and if its not adds the block to appendqueue
func (c *Core) CheckIfBlockIsAppended(block *types.Block) {
