...
type OverrideAccount struct {
	...
	MovePrecompileTo *common.InternalAddress     `json:"movePrecompileToAddress"`
}

// StateOverride is the collection of overridden accounts.
type StateOverride map[common.InternalAddress]OverrideAccount

func (diff *StateOverride) has(address common.InternalAddress) bool {
	_, ok := (*diff)[address]
	return ok
}

// Apply overrides the fields of specified accounts into the given state.
func (diff *StateOverride) Apply(statedb *state.StateDB, precompiles map[common.AddressBytes]vm.PrecompiledContract) error {
	if diff == nil {
		return nil
	}
	// Tracks destinations of precompiles that were moved.
	dirtyAddrs := make(map[common.InternalAddress]struct{})
	for addr, account := range *diff {
		...
		p, isPrecompile := precompiles[common.AddressBytes(addr.Bytes())]
		...
		if account.MovePrecompileTo != nil {
			...
			precompiles[account.MovePrecompileTo.Bytes20()] = p
			dirtyAddrs[*account.MovePrecompileTo] = struct{}{}
		}
		if isPrecompile {
			delete(precompiles, addr.Bytes20())
		}
		...
		if account.Balance != nil {
			...
			statedb.SetBalance(common.InternalAddress(addr), tracing.BalanceChangeUnspecified)
		}
		...
		if account.State != nil {
			statedb.SetStorage(addr.InternalAddress(), account.State)
		}
		// Apply state diff into specified accounts.
		if account.StateDiff != nil {
			for key, value := range account.StateDiff {
				statedb.SetState(addr.InternalAddress(), key, value)
			}
		}
	}
